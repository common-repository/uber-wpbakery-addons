...
switch ( $source ) {
	...
	if ( !$img_size ) { $img_size = 'medium'; }
	if ( $img_size == 'thumbnail' ) { $image = aq_resize($image,100,'',true,true,true); }
	elseif ( $img_size == 'medium' ) { $image = aq_resize($image,250,'',true,true,true); }
	elseif ( $img_size == 'large' ) { $image = aq_resize($image,500,'',true,true,true); }	
	elseif ( $img_size == 'full' ) { $image = $image; }
	elseif ( stripos ($img_size,'x') ) { 
		$explode = explode ('x',$img_size );
		$width = $explode[0];
		$height = $explode[1];
		$image = aq_resize($image,$width,$height,true,true,true);
	}

	$img['thumbnail'] = '<img src="'.esc_url( $image ).'">';
	break;
